...
public class WeatherGenerator {
    ...
    {        
        ...
        for(int i = 0; i < drywet.length; i++)
        {
            ...
            {
                for(int j = 2; j < drywet[i].length; j++)
                {
                    drywetProbability[j-2] = drywet[i][j];
                }
                break;
            }
        }
        ...
        {  
            ...
            {
                for(int j = 2; j < wetwet[i].length; j++)
                {
                    wetwetProbability[j-2] = wetwet[i][j];
                }
                break;
            }
        }
    ...
    public static int[] forecastGenerator(double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        for(int i = 0; i < forecast.length; i++)
        {
            ...
            else
            {
                ...
                else if(forecast[i] == WET)
                {
                    if(temp < wetwetProbability)
                    {
                        forecast[i] = WET;
                    }
                    else
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        {
            int temp = forecast[i] + forecast[i+1] + forecast[i+2] + forecast[i+3] + forecast[i+4] + forecast[i+5] + forecast[i+6];
            if(temp > max)
            {
                max = temp;
                maxIndex = i;
