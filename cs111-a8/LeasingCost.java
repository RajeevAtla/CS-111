public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        String type = description.substring(description.indexOf("type") + 4, description.indexOf(';', description.indexOf("type")));

        String name = description.substring(description.indexOf("name") + 4, description.indexOf(';', description.indexOf("name")));

        double due = Double.parseDouble(description.substring(description.indexOf("due") + 3, description.indexOf(';', description.indexOf("due"))));
        
        int length = Integer.parseInt(description.substring(description.indexOf("length") + 6, description.indexOf(';', description.indexOf("length"))));

        double monthlyCost = Double.parseDouble(description.substring(description.indexOf("monthly") + 7, description.indexOf(';', description.indexOf("monthly"))));

        double usage = Double.parseDouble(description.substring(description.indexOf("mile/unit") + 9, description.indexOf(';', description.indexOf("mile/unit"))));

        int mileageAllowance = Integer.parseInt(description.substring(description.indexOf("mileageAllowance") + 16, description.indexOf(';', description.indexOf("mileageAllowance"))));

        
        if(type.equals("electric"))
        {
            double chargerCost = Double.parseDouble(description.substring(description.indexOf("charger") + 7, description.indexOf('.', description.indexOf("charger"))));
            Fuel fuel = new Fuel(usage, chargerCost);
            Lease lease = new Lease(due, length, monthlyCost, mileageAllowance);
	...
	public static void computeCO2EmissionsAndCost(Vehicle[] vehicles, double gasPrice, double electricityPrice){
        ...
        // COMPLETE THIS METHOD
            ...
            for(int i = 0; i < vehicles.length; i++)
            {
                ...
                {
                    double CO2 = computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), Fuel.CO2EMITTED_ELECTRICITYCOMBUSTION);
                    vehicles[i].setCO2Emission(CO2);
                    fuelCost = computeFuelCost(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), electricityPrice);
                    vehicles[i].setFuelCost(fuelCost);
                }
                else
                {
                    double CO2 = computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), Fuel.CO2EMITTED_GASCOMBUSTION);
                    vehicles[i].setCO2Emission(CO2);
                    fuelCost = computeFuelCost(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), gasPrice);
                    vehicles[i].setFuelCost(fuelCost);
                }
